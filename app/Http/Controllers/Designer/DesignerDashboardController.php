...
class DesignerDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($timePeriod === 'ay' && isset($dateFrom, $dateTo)) {
            $distritutedQuery->whereBetween('distributed_at', [$dateFrom, $dateTo]);
        } else {
            $distritutedQuery->where('distributed_at', '>=', $dateFrom);
        }
        $distributedNewsletter = $distritutedQuery->count();
        ...
        if ($timePeriod === 'ay' && isset($dateFrom, $dateTo)) {
            $undistributeQuery->whereBetween('submitted_at', [$dateFrom, $dateTo]);
        } else {
            $undistributeQuery->where('submitted_at', '>=', $dateFrom);
        }
        $undistributedNewsletter = $undistributeQuery->count();
        ...
        // Fetch all categories with their articles and is_newsletter = 'yes' filtered by time period

        // Fetch all categories with their articles that are newsletters filtered by time period
        ...
        // Map categories to get the count of newsletter articles per category
        $categoriesWithArticleCount = $categories->map(function ($category) {
            // Count how many articles have is_newsletter = 'yes'
            $is_newsletter = $category->articles->count();
            return [
                'category_name' => $category->name,
                'is_newsletter' => $is_newsletter // Change this variable name if needed
            ];
        });
        ...
        $reportData = [
            'distributedNewsletter' => $distributedNewsletter,
            'undistributedNewsletter' => $undistributedNewsletter,
            'views' => $totalViews,
            'ratings' => $ratings,
            'comments' => $comments,
        ...
        return inertia('Designer/Dashboard', [
            ...
            'categoriesWithArticleCount' => $categoriesWithArticleCount,
